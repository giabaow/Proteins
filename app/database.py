...
class Recommendation(Base):
    """Single-row synthesis for the frontend Overview: the consolidated
    application + market-route recommendation for Proteins.1, drawn from the
    top-scoring companies' playbooks. Regenerated by pipeline.synthesize_recommendation."""
    __tablename__ = "recommendation"

    id = Column(Integer, primary_key=True)
    from_companies = Column(Text, default="[]")        # JSON list of names it was built from
    headline = Column(Text, default="")               # one-line takeaway
    applications = Column(Text, default="[]")         # JSON list of {application, rationale}
    market_route = Column(Text, default="[]")         # JSON list of {step, detail}
    sequence = Column(Text, default="")              # prose - the recommended order
    created_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))


class OpportunityPick(Base):
    """Single-row answer to: which specific disease-biomarker opportunity would
    benefit most from Proteins.1's ultra-sensitive, multiplexed platform, and
    which customer would pay for it first. Analytical, grounded where possible in
    the competitor landscape + article set. Regenerated by pipeline.pick_opportunity."""
    __tablename__ = "opportunity_pick"

    id = Column(Integer, primary_key=True)
    disease_area = Column(Text, default="")
    biomarker = Column(Text, default="")                 # the specific marker / panel
    why_it_fits = Column(Text, default="")               # why ultra-sensitivity + multiplex is decisive here
    unmet_need = Column(Text, default="")                # the clinical/measurement gap today
    competitive_gap = Column(Text, default="")           # why no analysed competitor already owns it
    first_customer = Column(Text, default="")            # the persona who pays first
    first_customer_why = Column(Text, default="")        # why them, why first
    runner_up = Column(Text, default="")                 # one alternative disease-biomarker, briefly
    evidence = Column(Text, default="[]")                # JSON list of {point, source}
    from_inputs = Column(Text, default="[]")             # JSON list - what it was built from
    created_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))
